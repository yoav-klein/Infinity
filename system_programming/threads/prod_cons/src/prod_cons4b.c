...
pthread_mutex_t g_mutex_prod;
pthread_mutex_t g_mutex_cons;
sem_t prod_sem;
sem_t cons_sem;
...
void *Producer(void *data)
{
	while(TIMES > __sync_fetch_and_add(&g_prod_count, 1))
	{
		...
		pthread_mutex_lock(&g_mutex_prod);
		...
		pthread_mutex_unlock(&g_mutex_prod);
	}
...
{	
	...
	while(TIMES > __sync_fetch_and_add(&g_cons_count, 1))
	{
		...
		pthread_mutex_lock(&g_mutex_cons);
			
		
		value = g_queue[g_cons_index % Q_SIZE];
		index = g_cons_index;
		++g_cons_index;
		...
		pthread_mutex_unlock(&g_mutex_cons);
		
		g_result[index] = value;
	}
...
int main()
{
	...
	status = pthread_mutex_init(&g_mutex_prod, NULL);
	if(0 != status)
	{
		printf("failure\n");
	...
	status = pthread_mutex_init(&g_mutex_cons, NULL);
	if(0 != status)
	{
		printf("failure\n");
	...
	for(i = 0; i < NUM_PRODUCERS; ++i)
	{
		...
		if(0 != status)
		{
			...
			pthread_mutex_destroy(&g_mutex_prod);
			pthread_mutex_destroy(&g_mutex_cons);
			return 1;
		}
	}
