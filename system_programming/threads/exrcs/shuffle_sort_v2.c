...
size_t Sort(const void *data, const void *args)
{
...
void RadixSort(struct thread_range *range, size_t longest)
{
	size_t i = 0;
	...
	for(i = longest - 1; i >= 0; --i)
	{
		...
		CountingSort(range->start, range->size, WORD_SIZE, Sort, i, ASCII_SIZE);
	}
}
...
size_t FindLongest(struct thread_range *range)
{
	...
	{
		curr_length = strlen(range->start[i]);
...
void *ThreadFunc(void *range)
{
	...
	RadixSort((struct thread_range*)range, longest);
