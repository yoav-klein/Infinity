...
size_t ByteValue(const void *data, const void *args)
{
	size_t curr_byte = (size_t)args;
	char *curr_word = (char*)data;
	size_t length = strlen(curr_word);
	
	if(length > curr_byte)
	{
		return curr_word[curr_byte];
	}
	return 0;
}

void MTRadixSort(struct thread_range *range, size_t longest)
{
	size_t i = 0;
	...
	for(i = longest - 1; i >= 0; --i)
	{
		...
		CountingSort(range->start_address, range->size, WORD_SIZE, ByteValue, (void*)i, ASCII_SIZE);
	}
}

size_t FindLongest(struct thread_range *range)
{
	size_t i = 0;
	size_t longest = 0;
	...
	{
		curr_length = strlen(range->start_address[i]);
...
void *ThreadFunc(void *range)
{
	size_t longest = FindLongest((struct thread_range*)range);
	
	MTRadixSort((struct thread_range*)range, longest);
